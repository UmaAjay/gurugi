...
def ui_info():
    st.markdown(f"""
	VIRTUAL ACHARYA USING NLP+OPENAI+HYDE
	""")
    ui_spacer(1)
    st.write("Made by GROUP", unsafe_allow_html=True)
    ui_spacer(1)
    st.markdown("""
		Our Group:
    ...
    st.markdown("""
		...
		""")
    ui_spacer(1)
    st.markdown('OUR MENTOR:')
    st.markdown('-->DIVYA MAAM')
...
def b_save():
	...
	help = "The file will be stored for about 90 days. Available only when using your Phone Number."
	if st.button('save encrypted index in Virtual Acharya', help=help):
		with st.spinner('saving to VirtualAcharya'):
			db.put(name, index)
